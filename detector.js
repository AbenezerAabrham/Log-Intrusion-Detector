...
// Hoisted constants for performance optimization
...
function analyzeLogs(rawText) {
  ...
  // Optimized loop: using for loop instead of forEach and hoisted constants
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0, pLen = PATTERN_ENTRIES.length; j < pLen; j++) {
      ...
      if (rule.regex.test(line)) {
        ...
        // Ensure finding appears only once in summary - optimized Map lookup
