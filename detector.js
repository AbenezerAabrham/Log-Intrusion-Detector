...
// Hoisted regular expressions to extract IP and HTTP Status from common access logs
...
function analyzeLogs(rawText) {
  ...
  // Avoid pre-filtering empty lines from log split to prevent intermediate array creation.
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0, patLen = PATTERN_ENTRIES.length; j < patLen; j++) {
      ...
      if (rule.regex.test(line)) {
        ...
        if (!finding) {
          findingsMap.set(entry.key, { ...rule, occurrence: 1 });
  ...
  for (let i = 0, ipsLen = ips.length; i < ipsLen; i++) {
    const ip = ips[i];
