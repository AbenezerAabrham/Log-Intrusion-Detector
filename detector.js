...
// Hoist static regexes and pattern entries to avoid recreation on every function call or loop iteration
...
function analyzeLogs(rawText) {
  ...
  // Avoid intermediate filtered array creation by splitting and skipping empty lines in the loop
  const rawLines = rawText.split('\n');
  const rawLinesLen = rawLines.length;
  const events = [];
  ...
  for (let i = 0; i < rawLinesLen; i++) {
    ...
    let flaggedReasons = null; // Lazy-initialize to avoid array allocation for safe lines
    ...
    // Regex Check 1: Iterating over defined patterns using a standard index-based loop
    ...
    if (flaggedReasons !== null && flaggedReasons.length > 0) {
      events.push({
        lineNum: totalLines, // Matches filtered index + 1
        ip,
        content: line,
        severity: lineType,
  ...
  // Heuristics 2: IP Aggregation checks using a standard loop over keys
