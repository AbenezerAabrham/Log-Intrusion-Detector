...
// Hoist patterns and regexes outside the hot path to avoid re-allocation and redundant lookups.
const PATTERN_ENTRIES = Object.entries(PATTERNS);
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

/**
 * Optimized log analysis engine.
 * - Uses a single pass for-loop for performance.
 * - Minimizes object allocations and redundant property lookups in the hot loop.
 */
function analyzeLogs(rawText) {
  ...
  // Single-pass processing using standard for-loop (faster than .forEach or .filter on large logs)
  for (let i = 0; i < totalRawLines; i++) {
    ...
    // Pattern Matching: iterate over pre-calculated PATTERN_ENTRIES
