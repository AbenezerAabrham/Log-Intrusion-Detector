...
// Hoist regex and pattern entries to avoid repeated allocations/lookups
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;
const PATTERN_ENTRIES = Object.entries(PATTERNS);

/**
 * ⚡ Bolt: Optimized log analysis engine.
 * Improvements:
 * - Hoisted static regexes and PATTERN_ENTRIES to avoid repeated allocation.
 * - Replaced .forEach loops with standard 'for' loops for better performance.
 * - Avoided intermediate array creation by processing lines in a single pass.
 * - Cached IP statistics lookups to minimize object access overhead.
 * - Implemented manual counter for flagged lines to avoid final array filtering.
 */
function analyzeLogs(rawText) {
  ...
  // Split once and handle empty lines during iteration to avoid intermediate array creation from .filter()
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0, plen = PATTERN_ENTRIES.length; j < plen; j++) {
    ...
    if (flaggedReasons.length > 0) {
      flaggedLineCount++;
      events.push({
        lineNum: i + 1, // Use original index for accurate UI line numbers
        ip,
        content: line,
        severity: lineType,
  ...
  for (let i = 0, iplen = ipKeys.length; i < iplen; i++) {
    const ip = ipKeys[i];
