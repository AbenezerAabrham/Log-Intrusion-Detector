...
// Hoist static definitions to avoid re-calculation in hot paths
...
function analyzeLogs(rawText) {
  ...
  // Single-pass processing of log lines
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    if (flaggedReasons.length > 0) {
      events.push({
        lineNum: totalLines, // Use totalLines for sequence consistency
        ip,
        content: line,
        severity: lineType,
  ...
  for (let i = 0, eLen = events.length; i < eLen; i++) {
    if (events[i].lineNum !== 'Agg') flaggedLineCount++;
  }
