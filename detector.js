...
// Pre-compiled static regexes and flat array structure for optimal execution speed
...
function analyzeLogs(rawText) {
  ...
  // Avoid intermediate split-filter array allocation by checking line emptiness inline.
  ...
  // Single-pass processing over raw log lines using fast indexed loop
  for (let i = 0; i < rawLines.length; i++) {
    ...
    let flaggedReasons = null; // Lazy initialization to reduce allocations for safe lines
    
    // basic extraction with static regexes and unary status conversion
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null;

    // Cached IP stats lookup
    ...
    for (let p = 0; p < PATTERN_ENTRIES.length; p++) {
      ...
      if (entry.regex.test(line)) {
        ...
        // track severity
        if (entry.severityVal > highestLineScore) {
          highestLineScore = entry.severityVal;
          lineType = entry.type;
        }
      }
    }
