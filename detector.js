...
// Hoist static regexes to avoid compiling them inside every call
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Flatten patterns structure for faster iterations and zero allocations during checking
...
function analyzeLogs(rawText) {
  ...
  // Split and do single-pass iteration inside standard index-based loop.
  // Avoid split + filter overhead.
  ...
  for (let i = 0; i < totalRawLines; i++) {
    ...
    // Lazy array allocation for flagged reasons
    ...
    // Cache the stats object for the IP
    ...
    // Iterate using fast sequential index-based loop over static rules
  ...
  // Fast loop for final IP heuristics
  ...
  // Manually count non-aggregated flagged lines
