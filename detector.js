...
// Hoist static regexes and entries to avoid repeated creation/iteration in hot paths
...
function analyzeLogs(rawText) {
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    if (flaggedReasons.length > 0) {
      flaggedLinesCount++;
      events.push({
        lineNum: totalLines, // Use totalLines (non-empty line counter) for lineNum
        ip,
        content: line,
        severity: lineType,
