...
// Optimization: Pre-flatten threat signatures for fast sequential iteration
const PATTERN_ENTRIES = Object.entries(PATTERNS).map(([key, rule]) => ({ key, rule }));

// Optimization: Hoist static regular expressions to avoid re-compilation per function call
...
function analyzeLogs(rawText) {
  ...
  // Optimization: Single split without intermediate filter array allocation
  ...
  // Optimization: Index-based loop to avoid function context overhead in .forEach
  for (let i = 0; i < lines.length; i++) {
    ...
    let flaggedReasons = null; // Optimization: Lazy allocation to reduce GC pressure on clean lines
    ...
    // Optimization: Local cached lookup for ipStats
    ...
    // Regex Check 1: Iterating over defined pre-flattened pattern entries
    for (let p = 0; p < PATTERN_ENTRIES.length; p++) {
      ...
      if (rule.regex.test(line)) {
        ...
        const key = entry.key;
        let existing = findingsMap.get(key);
        if (!existing) {
