...
// Optimization: Hoist static regular expressions and pre-flatten pattern entries outside analyzeLogs
// to avoid recreating regexes and allocating key-value pairs per function invocation.
...
function analyzeLogs(rawText) {
  ...
  // Optimization: Single split without intermediate filter array. Skip empty lines inside loop.
  ...
  // Optimization: Sequential index-based for loop avoids callback overhead and array allocations
  for (let index = 0; index < rawLines.length; index++) {
    ...
    let flaggedReasons = null; // Lazy-initialize array to avoid allocations for clean lines
    
    // basic extraction using static regular expressions and fast unary +
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null;

    // Cache ipStats[ip] reference
