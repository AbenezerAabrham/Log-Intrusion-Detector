...
// Hoist static regexes and pattern entries to avoid re-creation on every analysis call
...
function analyzeLogs(rawText) {
  ...
  // To avoid intermediate array creation, split but do not filter yet.
  // We will handle empty line checks efficiently within the main loop.
  ...
  for (let i = 0, len = ipStatsEntries.length; i < len; i++) {
    const entry = ipStatsEntries[i];
    const ip = entry[0];
    const stats = entry[1];
  ...
  const uniqueIps = ipStatsEntries.length;
...
// Escapes special HTML characters to prevent XSS in LogWatch dashboard
