...
// 1. Hoisted Regex patterns and flat pattern entries array outside the function to avoid repetitive construction/Object.entries calls on every execution.
// 2. Replaced the intermediate array created by split().filter() with a single split array and skipped empty elements in the index-based loop.
// 3. Converted all array iterations (.forEach, .filter) to highly optimized, sequential, index-based 'for' and 'for...in' loops.
// 4. Cached ipStats lookup (`ipStats[ip]`) inside the line iteration loop to prevent redundant property access.
// 5. Lazy-allocated the `flaggedReasons` array so that no memory allocation occurs for safe lines.
// 6. Utilized unary '+' instead of parseInt for fast string-to-number conversion of HTTP status codes.
// 7. Tracked `flaggedLinesCount` manually within the line loop to avoid calling filter() on the entire `events` array afterward.
...
function analyzeLogs(rawText) {
  ...
  const allLinesLen = allLines.length;
  const events = [];
