...
// Hoist static patterns/regexes/variables outside functions for high-performance log parsing.
// This prevents recreation and reallocation of arrays, objects, and RegExp instances on each run or line.
...
function analyzeLogs(rawText) {
  ...
  // Use standard index loop to split and filter out empty lines to avoid .filter() overhead.
  ...
  for (let i = 0, lineCount = lines.length; i < lineCount; i++) {
    ...
    let flaggedReasons = null; // Lazy-initialize to avoid 100k unnecessary empty array allocations
    
    // basic extraction
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null; // Unary '+' operator is much faster than parseInt

    // Cache nested object lookup
    ...
    // Regex Check 1: Iterating over defined patterns using cached flat entry array to bypass iterator allocation.
    for (let j = 0, plen = PATTERN_ENTRIES.length; j < plen; j++) {
      const entry = PATTERN_ENTRIES[j];
      if (entry.regex.test(line)) {
        if (!flaggedReasons) {
          flaggedReasons = [entry.name];
        } else {
  ...
  // Heuristics 2: IP Aggregation checks using fast standard loops
