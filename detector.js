...
// Hoist regexes and entries outside to avoid re-calculating on every analysis call
...
function analyzeLogs(rawText) {
  ...
  // Use standard for loop for better performance over large log files
  ...
  const ipStatEntries = Object.entries(ipStats);
  for (let i = 0; i < ipStatEntries.length; i++) {
    const [ip, stats] = ipStatEntries[i];
  ...
  const uniqueIps = ipStatEntries.length;
