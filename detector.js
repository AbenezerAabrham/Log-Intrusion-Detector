...
// Hoist static patterns and regular expressions outside the loop to avoid recreating them
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Convert the PATTERNS object to a flat array once to avoid repetitive Object.entries calls in high-frequency loops
const PATTERN_ENTRIES = Object.entries(PATTERNS).map(([key, rule]) => ({
  ...
  score: rule.score,
  type: rule.type,
  lineVal: rule.type === 'critical' ? 3 : rule.type === 'high' ? 2 : 1
}));
...
function analyzeLogs(rawText) {
  ...
  // Split lines without filtering upfront to avoid allocating extra intermediate arrays.
  // We will perform the trimming and empty check lazily during iteration.
  ...
  // Optimize pattern entry length query
  const patternsLength = PATTERN_ENTRIES.length;

  // High-performance index-based standard for loop
  for (let i = 0; i < rawLinesLength; i++) {
    ...
    // We lazy-allocate flaggedReasons to minimize object allocation pressure on clean/safe lines.
    let flaggedReasons = null;

    // Extract IP address and status
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch !== null ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch !== null ? +statusMatch[1] : null;

    // Cache nested object lookups in local scope
    ...
    // Efficient else-if chain instead of multiple separate conditionals
    if (status === 401) {
      stats[401]++;
    } else if (status === 403) {
      stats[403]++;
    ...
    // High-performance pattern iteration
    for (let p = 0; p < patternsLength; p++) {
      const entry = PATTERN_ENTRIES[p];
      if (entry.regex.test(line)) {
        if (flaggedReasons === null) {
          flaggedReasons = [];
        }
        flaggedReasons.push(entry.name);
        
        // Caching key in findingsMap avoids double hash lookups
        const key = entry.key;
        let finding = findingsMap.get(key);
        if (finding === undefined) {
          findingsMap.set(key, { ...entry.rule, occurrence: 1 });
        ...
        // track severity
        if (entry.lineVal > highestLineScore) {
          highestLineScore = entry.lineVal;
          lineType = entry.type;
        }
      }
    }
    ...
    if (flaggedReasons !== null) {
      flaggedLinesCount++;
      events.push({
        lineNum: i + 1, // correctly references the 1-based index in raw log split
        ip,
        content: line,
        severity: lineType,
  ...
  const ipList = Object.keys(ipStats);
  const ipListLength = ipList.length;

  for (let j = 0; j < ipListLength; j++) {
    const ip = ipList[j];
