...
// Hoist patterns & regexes outside high-frequency log analysis function to avoid recreating and parsing them repeatedly.
...
function analyzeLogs(rawText) {
  ...
  // Use fast, standard, index-based for-loop instead of .forEach to avoid function call overhead and closures.
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    let lineType = 'safe'; // default
    let highestLineScore = 0;
    let flaggedReasons = null; // Lazy allocate to avoid unnecessary empty array allocations
    
    // basic extraction
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null; // Use unary + for faster parsing than parseInt

    // Cache nested object lookups
