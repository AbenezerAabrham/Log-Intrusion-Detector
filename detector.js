...
// Optimization: Hoist static regular expressions and pattern entries outside analyzeLogs
// to avoid recreating RegExp objects and array entries on every function invocation.
...
function analyzeLogs(rawText) {
  ...
  // Optimization: Split once into lines without intermediate array filtering.
  // Empty lines are skipped inside the single-pass loop.
  ...
  // Optimization: Standard sequential index-based loop instead of .forEach
  for (let i = 0; i < rawLines.length; i++) {
    ...
    let flaggedReasons = null; // Lazy-allocation: only create array if flagged
    
    // Basic extraction with pre-compiled regex and fast unary '+' for status code
    ...
    // Regex Check 1: Iterating over hoisted PATTERN_ENTRIES with standard for loop
    ...
    // Save event if flagged and track flagged count inline
    if (flaggedReasons !== null) {
      flaggedLinesCount++;
      events.push({
        lineNum: nonEmptyCount,
        ip,
        content: line,
        severity: lineType,
  ...
  // Heuristics 2: IP Aggregation checks using for...in loop
