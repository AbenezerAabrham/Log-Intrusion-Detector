...
// Bolt Optimization: Pre-flatten PATTERNS into a static array to avoid Object.entries() allocations per scan
const PATTERN_ENTRIES = Object.entries(PATTERNS);

// Bolt Optimization: Hoist static regular expressions to avoid repeated re-compilation per line/scan
...
function analyzeLogs(rawText) {
  ...
  // Bolt Optimization: Avoid intermediate split().filter() array allocation by doing single split and skipping empty lines in-loop
  ...
  let riskTotal = 0;
  // Bolt Optimization: Track summary counters inline during line iteration to avoid post-scan e.filter()
  let totalLines = 0;
  let flaggedLinesCount = 0;

  // Bolt Optimization: Replace .forEach with fast sequential index-based for loop
  for (let i = 0; i < rawLines.length; i++) {
    ...
    // Bolt Optimization: Lazy-allocate flaggedReasons array only when a threat rule matches to avoid GC pressure on safe lines
    ...
    // Bolt Optimization: Fast unary + operator instead of parseInt
    const status = statusMatch ? +statusMatch[1] : null;

    // Bolt Optimization: Cache ipStats[ip] reference to avoid multiple property lookups
    ...
    // Bolt Optimization: Fast indexed loop over static pre-flattened PATTERN_ENTRIES
  ...
  // Bolt Optimization: Fast for...in loop instead of Object.entries().forEach
