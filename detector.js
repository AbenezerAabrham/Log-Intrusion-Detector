...
// Pre-calculate pattern entries for faster iteration
 ...
 * 1. Hoisted IP_REGEX, STATUS_REGEX, and PATTERN_ENTRIES outside analyzeLogs.
 * 2. Replaced .forEach() with standard for loops for performance.
 * 3. Avoided intermediate array for lines (now counts during main loop).
 * 4. Manual counter for flagged lines to avoid extra filter at end.
 * 5. Caching Object.entries(ipStats) results.
 */
function analyzeLogs(rawText) {
  ...
  // Avoid intermediate .filter() by handling empty lines in the loop
  ...
  for (let i = 0, len = rawLines.length; i < len; i++) {
    ...
    // Regex Check 1: Iterating over pre-calculated pattern entries
    ...
    if (flaggedReasons.length > 0) {
      flaggedLinesCount++;
      events.push({
        lineNum: i + 1, // Keep original line index (1-based)
        ip,
        content: line,
        severity: lineType,
