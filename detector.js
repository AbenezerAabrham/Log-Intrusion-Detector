...
function analyzeLogs(rawText) {
  ...
  // Single-pass log line analysis using standard for loop for performance
  for (let index = 0, len = lines.length; index < len; index++) {
    ...
    // IP and Status extraction using hoisted regexes
    ...
    // Iterate over hoisted pattern entries
  ...
  // Heuristics 2: IP Aggregation checks using standard for loop
  const ipEntries = Object.entries(ipStats);
  for (let i = 0, ipLen = ipEntries.length; i < ipLen; i++) {
