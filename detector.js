...
// Hoist static regexes and pre-flatten PATTERNS to avoid redundant allocations per invocation
...
function analyzeLogs(rawText) {
  ...
  // Single-pass line splitting & processing to eliminate intermediate split-filter array creation
  const rawLines = rawText.split('\n');
  const numRawLines = rawLines.length;
  ...
  // Sequential index-based loop for maximum throughput
  for (let i = 0; i < numRawLines; i++) {
    ...
    let flaggedReasons = null; // lazy-allocate array only when a threat signature matches
    
    // basic extraction using fast regex and unary plus for integer conversion
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null;

    // Cache nested ipStats object lookup
    ...
    // Regex Check 1: Iterating over defined patterns (SQLi, XSS, Path traversal, etc) using indexed loop
