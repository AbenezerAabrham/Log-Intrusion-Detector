...
// Bolt Optimization: Hoist static regular expressions and pre-flatten pattern entries for fast indexing
...
function analyzeLogs(rawText) {
  ...
  // Bolt Optimization: single split without intermediate array filtering; empty lines checked in loop
  ...
  // Bolt Optimization: high-performance index-based sequential loop for log line parsing
  for (let i = 0; i < lineCount; i++) {
    ...
    let flaggedReasons = null; // Bolt Optimization: lazy initialization avoids allocations on clean lines
    ...
    const status = statusMatch ? +statusMatch[1] : null; // Bolt Optimization: fast unary + instead of parseInt

    // Bolt Optimization: cached ipStats[ip] reference
    ...
    // Regex Check 1: Iterating over hoisted PATTERN_ENTRIES
