...
// Hoisted static regexes and pattern entries to avoid recreation overhead
...
function analyzeLogs(rawText) {
  ...
  // Avoid pre-filtering to eliminate intermediate arrays
  ...
  const patternLength = PATTERN_ENTRIES.length;
  ...
  for (let i = 0; i < linesLength; i++) {
    ...
    // Regex Check 1: Iterating over defined patterns with high-speed standard loop
    for (let p = 0; p < patternLength; p++) {
      ...
      if (entry.regex.test(line)) {
        ...
        let finding = findingsMap.get(entry.key);
        if (!finding) {
          const rule = PATTERNS[entry.key];
          finding = { ...rule, occurrence: 1 };
          findingsMap.set(entry.key, finding);
