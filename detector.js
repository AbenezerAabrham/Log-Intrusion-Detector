...
// Hoist Regex patterns and extract flat pattern entries for optimized lookup
...
function analyzeLogs(rawText) {
  ...
  // Pre-split the text into raw lines, then filter empty lines inside the single-pass loop
  // to avoid creating intermediate arrays from .filter() or pre-processing.
  ...
  // Process logs in a single highly-optimized sequential 'for' loop
  for (let i = 0, len = rawLines.length; i < len; i++) {
    const line = rawLines[i];
    // Equivalent to .filter(l => l.trim().length > 0)
    if (!line || line.trim().length === 0) {
      continue;
    }

    // Keep track of active non-empty line index (1-based index)
    totalLines++;
    const lineNum = totalLines;

    let lineType = 'safe'; // default
    let highestLineScore = 0;
    
    // Basic IP and Status extraction
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null;

    // Cache ipStats[ip] reference to avoid multiple hash lookups
    ...
    // Regex Check 1: Fast loop over static PATTERN_ENTRIES array
    let flaggedReasons = null; // lazy allocation
    for (let p = 0, pLen = PATTERN_ENTRIES.length; p < pLen; p++) {
      ...
      if (entry.regex.test(line)) {
        ...
        const existingFinding = findingsMap.get(entry.key);
        if (!existingFinding) {
          findingsMap.set(entry.key, {
            name: entry.name,
            type: entry.type,
            score: entry.score,
            desc: entry.desc,
  ...
  // Heuristics 2: IP Aggregation checks using fast for...in loop
