...
// Hoisted static regexes and pattern entries for maximized performance
...
function analyzeLogs(rawText) {
  ...
  // Split lines without intermediate .filter call to avoid extra array allocation
  ...
  // Fast sequential standard index-based for loop over log lines
  for (let i = 0; i < lines.length; i++) {
    ...
    let flaggedReasons = null; // Lazy initialization to reduce array creation overhead
    
    // basic extraction
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null; // Fast unary conversion to integer
    ...
    // Fast sequential loop over pattern entries
  ...
  const ipsKeys = Object.keys(ipStats);
  for (let i = 0; i < ipsKeys.length; i++) {
    const ip = ipsKeys[i];
  ...
  const uniqueIps = ipsKeys.length;
