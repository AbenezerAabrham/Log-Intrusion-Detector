...
// 1. Hoist static regexes out of the analyzeLogs function.
// 2. Pre-evaluate Object.entries(PATTERNS) to avoid calling it on every log file analysis.
// 3. Keep a pre-sorted and mapped array of rules to make matching extremely fast and GC-free.
...
function analyzeLogs(rawText) {
  ...
  // Performance Optimization 4: Split lines and avoid double-pass with `.filter` by checking inside the loop.
  ...
  // Performance Optimization 5: Standard sequential index-based `for` loop over `.forEach`.
  for (let i = 0; i < rawLinesLength; i++) {
    ...
    // Lazy-allocate flaggedReasons to save object creation.
    ...
    // Performance Optimization 6: Replace slower `parseInt(..., 10)` with fast unary `+` operator.
    const status = statusMatch ? +statusMatch[1] : null;

    // Performance Optimization 7: Cache nested lookups on ipStats.
    let ipStat = ipStats[ip];
    if (!ipStat) {
      ipStat = { count: 0, 401: 0, 403: 0, 404: 0, 500: 0 };
      ipStats[ip] = ipStat;
    }
    ipStat.count++;
    if (status === 401) ipStat[401]++;
    else if (status === 403) ipStat[403]++;
    else if (status === 404) ipStat[404]++;
    else if (status >= 500) ipStat[500]++;

    // Regex Check 1: Iterating over defined patterns (SQLi, XSS, Path traversal, etc) using pre-calculated PATTERN_ENTRIES
    ...
    if (flaggedReasons !== null) {
      events.push({
        lineNum: i + 1, // original line index (1-based)
        ip,
        content: line,
        severity: lineType,
  ...
  // Performance Optimization 8: Use `for...in` instead of `Object.entries(ipStats).forEach` to avoid overhead.
  ...
  // Performance Optimization 9: Count non-Agg flagged lines during standard iterations instead of double-passing with `.filter`.
  let flaggedLinesCount = 0;
  const numEvents = events.length;
  for (let j = 0; j < numEvents; j++) {
    if (events[j].lineNum !== 'Agg') {
      flaggedLinesCount++;
    }
  }
