...
/* --- HOISTED CONSTANTS & REGEXES FOR PERFORMANCE --- */
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

const PATTERN_ENTRIES = Object.keys(PATTERNS).map(key => ({
  key,
...
function analyzeLogs(rawText) {
  ...
  // Pre-split and process in a single pass to save on intermediate array filters and maps
  ...
  // Single-pass sequential loop for processing logs
  for (let index = 0; index < rawLinesLen; index++) {
    ...
    // Regex Check 1: Iterating over pre-compiled patterns
    let flaggedReasons = null;
    for (let pIdx = 0; pIdx < PATTERN_ENTRIES_LEN; pIdx++) {
      const rule = PATTERN_ENTRIES[pIdx];
