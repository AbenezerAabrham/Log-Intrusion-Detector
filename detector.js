...
// Hoist patterns and regular expressions outside of analyzeLogs for performance
...
function analyzeLogs(rawText) {
  ...
  // Pre-split the raw text into lines. We will filter out empty lines manually
  // inside the loop to avoid allocating an intermediate array.
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    // Lazy-initialize flaggedReasons to avoid array allocation overhead on clean lines
    ...
    // Replace slow parseInt with fast unary plus operator
    const status = statusMatch ? +statusMatch[1] : null;

    // Cache the lookup for the IP stats object
    ...
    // Regex Check 1: Iterating over defined patterns (SQLi, XSS, Path traversal, etc) using sequential loop
  ...
  // Heuristics 2: IP Aggregation checks using fast loop
  const ipKeys = Object.keys(ipStats);
  for (let i = 0, len = ipKeys.length; i < len; i++) {
    const ip = ipKeys[i];
