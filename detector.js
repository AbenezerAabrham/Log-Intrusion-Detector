...
// 1. Hoisted static regular expressions outside function scope to avoid recompilation on invocation.
// 2. Pre-flattened PATTERNS into static PATTERN_ENTRIES array for fast index-based looping.
...
function analyzeLogs(rawText) {
  ...
  // Performance Optimization: Split without immediate array filtering (.filter) to avoid extra allocation.
  ...
  // Performance Optimization: Standard sequential index-based 'for' loop for maximum performance across large inputs.
  for (let index = 0; index < allLines.length; index++) {
    ...
    let flaggedReasons = null; // Lazy-allocated array to eliminate 100k+ empty array creations
    
    // basic extraction using hoisted regexes and fast unary '+' number conversion
    const ipMatch = line.match(IP_REGEX);
    const ip = ipMatch ? ipMatch[0] : 'unknown';
    const statusMatch = line.match(STATUS_REGEX);
    const status = statusMatch ? +statusMatch[1] : null;

    // Cache ipStats[ip] lookup to avoid repeated object indexing
    ...
    // Performance Optimization: Fast index loop over pre-flattened pattern entries
  ...
  // Heuristics 2: IP Aggregation checks using fast for...in iteration
