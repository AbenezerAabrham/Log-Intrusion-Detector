...
// Hoisted static regular expressions to avoid compilation and allocation overhead inside the hot loop
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Hoisted and flattened pattern entries to avoid costly Object.entries() calls on every line
const PATTERN_ENTRIES = [];
for (const key of Object.keys(PATTERNS)) {
  PATTERN_ENTRIES.push({
    key,
    rule: PATTERNS[key],
    regex: PATTERNS[key].regex,
    name: PATTERNS[key].name,
    type: PATTERNS[key].type,
    score: PATTERNS[key].score,
    desc: PATTERNS[key].desc
  });
}
const PATTERN_ENTRIES_LEN = PATTERN_ENTRIES.length;
...
function analyzeLogs(rawText) {
  ...
  // Avoid creating an intermediate array through .filter by splitting once
  // and performing empty line validation inside a standard sequential for loop
  ...
  for (let i = 0; i < totalRawLines; i++) {
    ...
    totalLines++; // Active processed non-empty line count, matches original indexing

    let lineType = 'safe'; // default
    let highestLineScore = 0;
    let flaggedReasons = null; // Lazy-allocated to avoid object allocation overhead for safe lines
    ...
    // Fast lookup cache for ip statistics
    ...
    // Regex Check 1: Iterating over defined patterns using fast index-based for loop
    for (let p = 0; p < PATTERN_ENTRIES_LEN; p++) {
      ...
      if (entry.regex.test(line)) {
        ...
        const lineVal = type === 'critical' ? 3 : type === 'high' ? 2 : 1;
  ...
  // Heuristics 2: IP Aggregation checks using fast for...in traversal
