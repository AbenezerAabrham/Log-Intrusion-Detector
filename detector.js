...
// Hoist static regexes and pattern entries to avoid recreation in hot loops
...
function analyzeLogs(rawText) {
  ...
  // Use a single pass over lines to avoid multiple array allocations
  ...
  for (let i = 0; i < allLines.length; i++) {
    ...
    for (let j = 0; j < PATTERN_ENTRIES.length; j++) {
      ...
      if (rule.regex.test(line)) {
        ...
        // Ensure finding appears only once in summary, optimize Map lookup
  ...
  // Heuristics 2: IP Aggregation checks using standard loop
  const ipEntries = Object.entries(ipStats);
  for (let i = 0; i < ipEntries.length; i++) {
    const ipEntry = ipEntries[i];
    const ip = ipEntry[0];
    const stats = ipEntry[1];
