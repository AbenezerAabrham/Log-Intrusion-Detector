...
// Hoisted regexes to avoid re-compilation in every analyzeLogs call
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Pre-compiled PATTERN_ENTRIES for hot-loop pattern checking
...
function analyzeLogs(rawText) {
  ...
  // Single-pass processing: avoiding intermediate arrays from filter()
  for (let i = 0; i < rawLinesLen; i++) {
    ...
    // Lazy-initialize flaggedReasons to avoid array allocation for clean logs
    ...
    // Fast lookup caching for ipStats
    ...
    // Regex checking utilizing flat sequential loop over pre-compiled rule entries
    const patternsLen = PATTERN_ENTRIES.length;
    for (let p = 0; p < patternsLen; p++) {
      const rule = PATTERN_ENTRIES[p];
  ...
  // Heuristics 2: IP Aggregation checks using fast for...in sequential loops
