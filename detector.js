...
// Hoisted constants for optimized log analysis
...
function analyzeLogs(rawText) {
  ...
  // Split lines without filtering up-front to avoid intermediate array allocation
  ...
  // Single-pass loop over all log lines
  for (let index = 0; index < lines.length; index++) {
    ...
    // Iterating over defined patterns with standard for-loop and cached pattern entries
    for (let i = 0; i < PATTERN_ENTRIES.length; i++) {
      const rule = PATTERN_ENTRIES[i];
      ...
      if (rule.regex.test(line)) {
        ...
        let finding = findingsMap.get(rule.key);
        if (!finding) {
  ...
  // Heuristics 2: IP Aggregation checks with optimized loop
