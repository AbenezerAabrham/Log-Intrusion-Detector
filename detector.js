...
// Pre-compute regexes and pattern entries to avoid overhead during log analysis
...
function analyzeLogs(rawText) {
  ...
  // Single pass over lines to avoid intermediate array creation from .filter()
  for (let i = 0; i < rawLines.length; i++) {
    ...
    // Heuristic 1: Pattern matching using standard for-loop for speed
    ...
    if (flaggedReasons.length > 0) {
      flaggedLinesCount++;
      events.push({
        lineNum: totalLines, // Use processed line counter to maintain original behavior
        ip,
        content: line,
        severity: lineType,
