...
// Hoisted constants for high-performance log analysis
...
function analyzeLogs(rawText) {
  ...
  // Performance optimization: split directly and iterate with index-based loop
  // to avoid creating intermediate arrays via .filter() and callback overhead in .forEach()
  ...
  for (let i = 0; i < len; i++) {
    ...
    let flaggedReasons = null; // Lazy-allocated array to avoid creating objects for safe lines
    ...
    const status = statusMatch ? +statusMatch[1] : null; // Fast unary integer conversion

    // Cache nested lookup for ipStats
  ...
  // Heuristics 2: IP Aggregation checks using standard for...in loop
