...
function analyzeLogs(rawText) {
  ...
  // BOLT OPTIMIZATION: Avoid extra array allocation and pass with .filter().
  // Instead, we skip empty lines within the main loop.
  ...
  // BOLT OPTIMIZATION: Hoist pattern entries to avoid repeated Object.entries() calls.
  const patternEntries = Object.entries(PATTERNS);
  const patternCount = patternEntries.length;
  let lineNum = 0;

  // BOLT OPTIMIZATION: Use a standard for-loop instead of .forEach() for better performance in hot paths.
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (!line || line.trim().length === 0) continue;

    lineNum++; // Tracks the semantic line number (non-empty lines)
