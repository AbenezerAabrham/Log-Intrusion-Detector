...
// Hoisted and pre-compiled regexes & PATTERN_ENTRIES for maximum performance
...
function analyzeLogs(rawText) {
  ...
  // OPTIMIZATION: Avoid intermediate split-filter arrays by processing split lines in a single-pass loop.
  ...
  // OPTIMIZATION: Fast sequential standard index-based `for` loops.
  for (let i = 0; i < len; i++) {
    ...
    let flaggedReasons = null; // OPTIMIZATION: Lazy-initialize array to avoid unnecessary allocations
    ...
    // OPTIMIZATION: Use fast unary `+` instead of `parseInt`
    const status = statusMatch ? +statusMatch[1] : null;

    // OPTIMIZATION: Cache the ipStats nested object lookup
    ...
    // OPTIMIZATION: Iterate over pre-compiled flat PATTERN_ENTRIES array using index loop instead of Object.entries().forEach
    ...
    if (flaggedReasons !== null && flaggedReasons.length > 0) {
      flaggedLinesCount++;
      events.push({
        lineNum: i + 1, // OPTIMIZATION: Use loop index for precise source line reference
        ip,
        content: line,
        severity: lineType,
  ...
  // OPTIMIZATION: Use index loop instead of Object.entries().forEach
  const ipKeys = Object.keys(ipStats);
  const ipKeysLen = ipKeys.length;
  for (let k = 0; k < ipKeysLen; k++) {
    const ip = ipKeys[k];
  ...
  return {
    ...
    summary: {
      totalLines,
      flaggedLines: flaggedLinesCount, // OPTIMIZATION: inline manual counter avoids events.filter allocation
      uniqueIps: ipKeysLen
    }
  };
}
