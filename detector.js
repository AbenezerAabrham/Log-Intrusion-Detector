...
// Hoisted regular expressions to avoid recompilation per log line / invocation
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Pre-flatten PATTERNS into a static array for fast indexed iteration
...
function analyzeLogs(rawText) {
  ...
  // Avoid intermediate split-filter array creation; check empty lines in loop
  ...
  for (let i = 0; i < numLines; i++) {
    ...
    let flaggedReasons = null; // Lazy-allocated to reduce GC overhead for clean lines
    ...
    // Regex Check 1: Iterating over defined threat signatures with fast index loop
    for (let j = 0; j < numPatterns; j++) {
      const p = PATTERN_ENTRIES[j];
      if (p.regex.test(line)) {
        if (!flaggedReasons) flaggedReasons = [];
        flaggedReasons.push(p.name);
        
        // Ensure finding appears only once in summary
        const existingFinding = findingsMap.get(p.key);
        if (!existingFinding) {
          findingsMap.set(p.key, { ...p.rule, occurrence: 1 });
          riskTotal += p.score; // only add score once per pattern type for overall log
        } else {
          existingFinding.occurrence++;
        }

        // Track severity
        if (p.lineVal > highestLineScore) {
          highestLineScore = p.lineVal;
          lineType = p.type;
        }
      }
    }
