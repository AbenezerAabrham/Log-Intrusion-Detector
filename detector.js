...
// Hoisted for performance to avoid re-creation or re-traversal in hot loops
...
function analyzeLogs(rawText) {
  ...
  // Bolt Optimization: Use standard for loop instead of forEach for better performance on large log sets
  for (let i = 0; i < lines.length; i++) {
    ...
    // Cache ipStats[ip] to avoid repeated lookups
    if (!ipStats[ip]) {
    ...
    // Bolt Optimization: Use PATTERN_ENTRIES and standard for loop
  ...
  for (let i = 0; i < ipStatsEntries.length; i++) {
    const ip = ipStatsEntries[i][0];
    const stats = ipStatsEntries[i][1];
