...
// Hoisted constants to avoid redundant creation/calculation in the analysis loop
...
function analyzeLogs(rawText) {
  ...
  // Optimized loop: using standard for loop and avoiding forEach/entries in hot path
  for (let index = 0; index < lines.length; index++) {
    ...
    for (let j = 0; j < PATTERN_ENTRIES.length; j++) {
      ...
      if (rule.regex.test(line)) {
        ...
        // Ensure finding appears only once in summary, optimized lookup
  ...
  for (let i = 0; i < ipEntries.length; i++) {
    const ip = ipEntries[i][0];
    const stats = ipEntries[i][1];
