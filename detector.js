...
// Pre-calculate PATTERNS entries to avoid calling Object.entries in every line iteration
const PATTERN_ENTRIES = Object.entries(PATTERNS);

// Regexes hoisted outside the function to avoid recreation
...
function analyzeLogs(rawText) {
  ...
  // Avoid unnecessary filter and secondary array by using a single-pass loop
  ...
  // Optimized loop using standard for-loop and hoisting logic
  ...
  for (let i = 0; i < ipStatsEntries.length; i++) {
    ...
    if (fails >= 4) {
      ...
      if (findingsMap.get(bruteKey) === undefined) {
    ...
    if (stats[404] >= 6) {
      ...
      if (findingsMap.get(scanKey) === undefined) {
