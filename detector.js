...
// Hoist static regexes to avoid re-compiling on every analyzeLogs execution
const IP_REGEX = /\b(?:\d{1,3}\.){3}\d{1,3}\b/;
const STATUS_REGEX = /\s([2345]\d{2})\s/;

// Pre-flatten PATTERNS into a static array to avoid Object.entries allocations inside hot line processing loops
...
function analyzeLogs(rawText) {
  ...
  // Avoid intermediate split-and-filter array allocations by using single-pass index loop
  ...
  for (let index = 0; index < rawLines.length; index++) {
    ...
    let flaggedReasons = null; // Lazy-allocated only when a pattern matches
    ...
    // Cache ipStats[ip] lookup to avoid repeated object property accesses
    ...
    // Regex Check 1: Iterating over defined pre-flattened patterns (SQLi, XSS, Path traversal, etc)
